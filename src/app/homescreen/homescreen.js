import { Link } from "expo-router";
import { StatusBar } from "expo-status-bar";
import React from 'react';
import { Text, TextInput, View, TouchableOpacity, StyleSheet, Image, FlatList } from "react-native";
import Icon from 'react-native-vector-icons/FontAwesome';


const HomeScreen = () => {
  // Dados de exemplo para cada seção
  const recentGames = [{ id: '1', title: 'Game 1' }, { id: '2', title: 'Game 2' }];
  const friendsGames = [{ id: '3', title: 'Game 3' }, { id: '4', title: 'Game 4' }];
  const featuredGames = [{ id: '5', title: 'Game 5' }, { id: '6', title: 'Game 6' }];

  const renderGameItem = ({ item }) => (
    <View style={styles.gameItem}>
       <Text style={styles.gameTitle}>{item.title}</Text>
    </View>
  );

  return (
    <View style={styles.container}>

      <Text style={styles.title}>Entrar</Text>
      <Text style={styles.sectionTitle}>Recentemente Adicionados</Text>
      <View style={styles.underline} />
      <FlatList
        data={recentGames}
        renderItem={renderGameItem}
        keyExtractor={item => item.id}
        horizontal
        showsHorizontalScrollIndicator={false}
      />

      <Text style={styles.sectionTitle}>Adicionados por Amigos</Text>
      <View style={styles.underline} />
      <FlatList
        data={friendsGames}
        renderItem={renderGameItem}
        keyExtractor={item => item.id}
        horizontal
        showsHorizontalScrollIndicator={false}
      />

      <Text style={styles.sectionTitle}>Destaques da Semana</Text>
      <View style={styles.underline} />
      <FlatList
        data={featuredGames}
        renderItem={renderGameItem}
        keyExtractor={item => item.id}
        horizontal
        showsHorizontalScrollIndicator={false}
      />
    </View>
    
  );
};

const styles = StyleSheet.create({
  container: {
    flex: 1,
    padding: 20,
    backgroundColor: '#fff',
    marginTop: 8,

  },
  sectionTitle: {
    fontSize: 20,
    fontWeight: 'bold',
    marginBottom: 1,
    color: '#a4a3aa',
    alignItems: 'flex-start',
    marginTop: 20 
  },
  gameItem: {
    width: 150, 
    height: 150, 
    marginRight: 15,
    marginTop: 20, 
    padding: 10,
    backgroundColor: '#D8BFD8',
    borderRadius: 8,
    alignItems: 'center', 
    alignItems: 'center',
  },
  gameTitle: {
    fontSize: 16,
    color: 'white'
  },

  container: {
    flex: 1,
    backgroundColor: "#1C1A2B",
    alignItems: "'flex-start'",
    justifyContent: "center",
    padding: 20,
    marginTop: 8,
  },

  underline: {
    width: '90%',          
    height: 1,             
    backgroundColor: 'white', 
    
  },
});

export default HomeScreen;